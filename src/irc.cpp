...
// Copyright (c) 2011-2012 EstheticaCoin Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("EstheticaCoin-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #EstheticaCoinTEST0\r");
            Send(hSocket, "WHO #EstheticaCoinTEST0\r");
        } else {
            // randomly join #EstheticaCoin00-#EstheticaCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #EstheticaCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #EstheticaCoin%02d\r", channel_number).c_str());
        }
