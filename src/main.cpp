...
uint256 hashGenesisBlock("0x5126f4814f17221725a8704fab0508a2716025bcf2bbadba0aa7651454e5b4a1");
...
const string strMessageMagic = "EstheticaCoin Signed Message:\n";
...
{
    // EstheticaCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // EstheticaCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "est genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1495633915;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1124384;
...
unsigned char pchMessageStart[4] = { 0xda, 0xcb, 0xa1, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EstheticaCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EstheticaCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EstheticaCoinMiner(CWallet *pwallet)
{
    printf("EstheticaCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("EstheticaCoin-miner");
    ...
    try { loop {
        ...
        printf("Running EstheticaCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EstheticaCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EstheticaCoinMiner, pwallet));
}
